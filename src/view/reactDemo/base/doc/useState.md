...
另外，这个页面上展示的详情的形式，比如说同样的数据我们可以以列表形式展示，也可以表格类型展示，甚至可以以echarts图图表展示，那么这里这个控制展示形式变化的值也需要用一个状态去维护，比如说`mode`,`mode`有`list`,`table`,`echarts`三种值，数据不变的情况改变mode的值同样会让页面重新渲染。
