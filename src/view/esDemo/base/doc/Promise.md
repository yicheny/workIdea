...
# 解决什么问题？
promise是ES6提出的新的异步解决方案，在此之前异步的解决方案是回调，通过回调表达程序异步和管理并发有两个主要缺陷：缺乏线性顺序和缺乏信任。

回调第一个缺点是它不符合人类的思考模式，不是按执行顺序从上到下线性执行的。

第二个缺点，是当我们将回调传递给第三方函数【或自己编写的高阶函数时】，回调时是在高阶函数内部执行的，如果不去看实现细节，你就没办法了解回调是如何调用的，它是同步还是异步？有没有执行？执行几次？什么时候执行？这些你都只能通过查看高阶函数内部实现知道，并且你将回调函数的控制权交由高阶函数时你就失去了控制权，原本我们只是想知道异步什么时候结束，然后执行即可。

回调使得代码耦合起来了，我们必须去查看细节才可以了解其具体调用情况，这增加了开发的负担，这不是我们所希望的。更重要的是，我们失去了回调的控制权，一旦第三方函数或我们自己的高阶函数出现问题，会怎样？

一旦我们在其他函数回调使用出现问题，尤其是第三方函数，会极难修改，基本难以修改只能提Issue依托第三方修复【不过也不是毫无办法，我们可以做一些检查，但这会引发另一个问题，这套检查系统本身就需要一定的水平去实现，其实现难度甚至高于一般的业务开发，而且这套检查带来了多余的开发以及维护成本，会非常恶心。】

即便是在自己开发的高阶函数中想要修复也不是简单的一件事，因为回调的出现就意味着耦合度的增加，在一个高度耦合的函数内想要改动是很不容易的。

我们将回调交给第三方或高阶函数执行，因失去了控制权，我们被迫相信第三方或高阶函数，信任一旦出现问题，我们难以修复，信任问题的本质在于失去了控制权，如果我们可以自己控制回调的执行，还会有信任问题吗？ 

Promise最关键的一点正是帮助开发者取回了控制权，从而解决了回调带来的信任问题。

基于刚刚的解释，回调就是将未来要做的事交托给第三方，而第三方如何调用，何时调用，会不会调用这些我们都难以确保，一旦出现信任问题，我们难以修复。
...
### 处理Promise未捕获
> 注，此错误已被官方修复，其最后的错误会被全局抛出，下面就当作思路看一下吧，官方和第二个思路终结函数done基本相同<br/>
> [官方代码](https://github.com/domenic/promises-unwrapping.git)
...
> 检测试，在原生Promise中catch会如这里所说的done方法一样抛出一个全局错误
...
static all = (list)=> {
    return new Promise((resolve,reject)=>{
        ...
        list.forEach((el,i)=>{
            el.then(res=>{
                evts[i]=res;
                if(evts.length===list.length && !evts.includes(undefined) return resolve(evts);//注意，这样会导致undefined不能透传
            },reject);
        });
    })
}
