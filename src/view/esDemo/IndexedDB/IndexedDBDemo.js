...
function IndexedDbDemo(props) {
    const [db] = useState(new IndexedDbClient());

    return <Container header='IndexedDbDemo'>
        <Button type='primary' onClick={()=>db.add({name:'123',attack:1223,begin:12312})}>新增数据</Button>
        <Button type='primary' onClick={()=>db.edit(2,{name:'la',hp:'ddd'})}>编辑数据</Button>
        <Button type='primary' onClick={()=>db.query(2)}>查询数据</Button>
        <Button type='primary' onClick={()=>db.del(3)}>删除数据</Button>
    </Container>;
}
