...
function SystemSwitch(props) {
    ...
    return <Container header='进制转换'>
        <div>
            <h3>十进制转二进制:</h3>
            <TextInput type='number' value={tenNum} onChange={(e)=>setTenNum(Number(e.target.value))}/>
            <span style={{margin:'0 12px'}}>{tenSOther()}</span>
        </div>
        <div>
            <h3>二进制转十进制:</h3>
            <TextInput type='number' value={twoNum} onBlur={(e)=>setTwoNum(Number(e.target.value))}/>
